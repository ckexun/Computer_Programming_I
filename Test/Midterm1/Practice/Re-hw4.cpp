...
/*int main()
{
    int multiplicandExpon[20] = { 2,8,14 };
    int multiplicandCoef[20] = { 1,-2,3};
    int multiplierExpon[20] = { 4,10,16 };
    int multiplierCoef[20] = { 2,2,2};
    int productSize = 1,productCoef[20]{}, productExpon[20]{};
    int multiplicandSize = 19, multiplierSize = 19, pd = 19, rd = 19;
    while (multiplicandSize > 1 && multiplicandCoef[multiplicandSize - 1] == 0)
        multiplicandSize--;
    while (multiplierSize > 1 && multiplierCoef[multiplierSize - 1] == 0)
        multiplierSize--;
    multiplication(multiplicandCoef, multiplicandExpon, multiplicandSize, multiplierCoef, multiplierExpon, multiplierSize
        ,productCoef, productExpon, productSize);
    for (int i = productSize - 1; i >= 0; i--) {
        cout << productCoef[i] << "X^" << productExpon[i] << " ";
    }
}*/
               ...
               int remainderCoef[], int remainderExpon[], int &remainderSize )
{
   ...
   for( int i = quotientSize - 1; i >= 0; i-- )
   {
       ...
       if (isZero(remainderExpon, remainderSize) || remainderExpon[remainderSize - 1] < divisorExpon[divisorSize - 1]) {
           if (i != 0)
                for (int j = 0; j < quotientSize - i; j++) {
