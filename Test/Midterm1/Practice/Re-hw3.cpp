                     ...
                     int multiplicandDegree, int multiplierDegree, int &productDegree )
{
    ...
    productDegree = multiplierDegree + multiplicandDegree;
...
/*int main()
{
    int dividend[20] = { -4,-3,-2,1 };
    int divisor[20] = {-3,1};
    int quotient[20] = {};
    int remainder[20] = {};
    int dividendDegree = 19, divisorDegree = 19, quotientDegree = 19, remainderDegree = 19;
    while (dividendDegree > 0 && dividend[dividendDegree] == 0)
        dividendDegree -= 1;
    while (divisorDegree > 0 && divisor[divisorDegree] == 0)
        divisorDegree -= 1;
               ...
               int dividendDegree, int divisorDegree, int &quotientDegree, int &remainderDegree )
{
   ...
   quotientDegree = dividendDegree - divisorDegree; //quotient最高次方
   for (int i = quotientDegree; i >= 0; i--) {
       quotient[i] = remainder[remainderDegree] / divisor[divisorDegree];//算出商
       int buffer[20]{},bufferDegree=20;//宣告buffer                              //放入buffer
       int q2[20] = {}, q2Degree = quotientDegree;
       q2[i] = quotient[i];
       multiplication(q2, divisor, buffer, q2Degree, divisorDegree, bufferDegree);
       subtraction(remainder, buffer, remainderDegree, bufferDegree); //remainder-=buffer
       if (remainderDegree < divisorDegree)
           break;
       if (divisorDegree == 0 && remainderDegree == 0 && remainder[0] < divisor[0])
           break;
   }
