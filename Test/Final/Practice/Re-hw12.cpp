...
void inputDepartureDate( char date[], int &departureDayWeek )
{
   ...
   cin >> choise;
   Date departureDate{};
   departureDate.month = currentDate.month + choise - 1;
   departureDate.year = currentDate.year;
   ...
   if (departureDate.month == 1 || departureDate.month == 3 || departureDate.month == 5 || departureDate.month == 7 ||
       ...
       lastDay = leapYear(departureDate.year) ? 29 : 28;
   else
       lastDay = 30;
   ...
   cout << endl << "Day (" << day << " ~ " << lastDay << "): ";
   cin >> departureDate.day;
   cout << endl;

   departureDayWeek = difference(departureDate, currentDate) + structuredTime.tm_wday;
   ...
   while (departureDate.month > 0) {
       tmp[i] = departureDate.month % 10 + '0';
...
int difference( Date departureDate, Date currentDate )
{
    ...
    month[2] = leapYear(currentDate.year) ? 29 : 28;
    ...
    if (departureDate.year != currentDate.year){
        ...
        month[2] = leapYear(departureDate.year) ? 29 : 28;
    }
    for (int i = 0; i < departureDate.month; i++)
        departureDay += month[i];

    int weekDay = (departureDay - currentDay) % 7;
    return weekDay;
}
...
void loadFlightSchedules( Flight flights[], int &numFlights )
{
    fstream load("Flight Schedule.dat", ios::in | ios::binary);
    while (load.read(reinterpret_cast<char*>(&flights[numFlights]), sizeof(Flight)))
        numFlights++;
    load.close();
}
...
{
    fstream load("Flight Schedule.dat", ios::in | ios::binary);
    while (load.read(reinterpret_cast<char*>(&flights[0]), sizeof(Flight))) {
        if (departureAirport == flights[0].departureAirport && arrivalAirport == flights[0].arrivalAirport &&
...
void loadFlightSeats( FlightSeats flightSeats[], int &numFlightSeats, char departureDate[],
     int departureAirport, int arrivalAirport )
{
    fstream load("Flight Seats.dat", ios::in | ios::binary);
    while (load.read(reinterpret_cast<char*>(&flightSeats[0]), sizeof(FlightSeats))) {
        if (departureAirport == flightSeats[0].departureAirport && arrivalAirport == flightSeats[0].arrivalAirport &&
            strcmp(departureDate, flightSeats[0].date) == 0) {
            numFlightSeats++;
...
void modifyFlightSeatsFile( Reservation &reservation, int totalNumTickets )
{
    fstream change("Flight Seats.dat", ios::in | ios::out | ios::binary);
    int move = 0;
    FlightSeats flight{};
    while (change.read(reinterpret_cast<char*>(&flight), sizeof(FlightSeats))) {
        if (strcmp(flight.flightNo, reservation.flightNo) == 0 && strcmp(flight.date, reservation.date) == 0) {
            flight.availableSeats -= totalNumTickets;
            change.seekp(sizeof(FlightSeats) * move);
            change.write(reinterpret_cast<char*>(&flight), sizeof(Flight));
            break;
        }
        move++;
    }
...
void refundApplication()
{
   ...
   ofstream deleteR("Reservations.dat", ios::out | ios::binary);
   int j = 0;
   for (int i = 0; i < numReservations; i++) {
       bool in = true;
       if (strcmp(reservations[i].id, id) == 0) {
           ...
           if (j == choice) {
               ...
               int totalNumTickets = 0;
               for (int k = 1; k < 8; k++)
                   totalNumTickets += reservations[i].tickets[k];
               
               modifyFlightSeatsFile(reservations[i], -totalNumTickets);
           }
       }
       if (in)
           deleteR.write(reinterpret_cast<char*>(&reservations[i]), sizeof(Reservation));
   }
...
void loadReservations( Reservation reservations[], int &numReservations )
{
    fstream load("Reservations.dat", ios::in | ios::binary);
    numReservations = 0;
